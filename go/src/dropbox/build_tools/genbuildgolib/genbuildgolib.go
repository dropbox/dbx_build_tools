...
// PopulateSrcsAndDeps populates the "srcs" and "deps" attributes to
// write to the BUILD file
func PopulateSrcsAndDeps(pkg *build.Package,
	cgoIncludeFlags,
	cgoCXXFlags,
	cgoLinkerFlags,
	deps,
	srcs,
	cgoSrcs []string) ([]string, []string, []string, []string, []string, []string, error) {
...
// WriteToBuildConfigFile performs the actual file write operation
...
// which doesn't need to be listed as a dependency.
func IsBuiltinPkg(pkgName string, golangPkgs map[string]struct{}, isVerbose bool) (bool, bool) {
...
// PopulatePackageInfo prepares the package for config generation.
// This includes some sanity checks, skip checks and gathers the
// imports of the package.
func PopulatePackageInfo(
	goPkgPath,
	workspace,
	goSrc string,
	isVerbose,
	...
	visitStack = append(visitStack, goPkgPath)
	defer func() { visitStack = visitStack[:len(visitStack)-1] }()
	...
	if config["build_tags"] != nil {
		if !IsWhitelistedForBuildTags(goPkgPath) {
			return nil, fmt.Errorf("You must add %s to whitelistForBuildTags in gen-build-go.go to enable build_tags.", goPkgPath)
		}
...
func InitializeSrcsAndCGOSrcs(pkg *build.Package) ([]string, []string) {
	srcs := []string{}
