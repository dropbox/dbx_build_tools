...
func (nw *numericWeighter) AppendNext(buf []Elem, s []byte) (ce []Elem, n int) {
	...
	// NOTE: This part is different from original package, we changed the logic a little
	// bit to fix a bug.
	// We filed the issue on github, re-pull from the repo when this bug get fixed.
	// See https://github.com/golang/go/issues/25554
	nextByte := byte('0')
	if n < len(s) {
		nextByte = s[n]
	}
